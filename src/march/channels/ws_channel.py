"""WSChannel — Embedded HTTP/WS server channel for the frontend chat.
...
Runs an aiohttp server, handles image resizing, PDF extraction, voice
transcription, and stream persistence (even if frontend disconnects
mid-stream).

Configuration is read from config.yaml under `channels.ws_proxy`.
Legacy `plugins.ws_proxy` is supported with a deprecation warning.
...
class WSChannel(Channel):
    """Embedded HTTP/WS server channel for the frontend chat — pure I/O adapter.

    All agent execution, session resolution, message persistence, and draft
    handling are delegated to the Orchestrator.  This channel only handles:
    ...
    Configuration (config.yaml → channels.ws_proxy):
    ...
    def __init__(self, config: Any = None) -> None:
        ...
        # Apply config if provided at construction time
        if config is not None:
            self._apply_config(config)

    def _apply_config(self, cfg: Any) -> None:
        """Apply a config object (Pydantic model) to instance attributes."""
    ...
    def _load_config(self, app: Any) -> None:
        """Load channel config from app.config.channels.ws_proxy.

        Falls back to plugins.ws_proxy (deprecated) with a warning.
        """
        cfg = None
        if hasattr(app, "config") and app.config:
            # Primary: channels.ws_proxy (new location)
            channels_cfg = getattr(app.config, "channels", None)
            if channels_cfg:
                cfg = getattr(channels_cfg, "ws_proxy", None)
            # Fallback: plugins.ws_proxy (deprecated)
            if cfg is None:
                plugins_cfg = getattr(app.config, "plugins", None)
                if plugins_cfg:
                    old_cfg = getattr(plugins_cfg, "ws_proxy", None)
                    if old_cfg is not None:
                        warnings.warn(
                            "plugins.ws_proxy config is deprecated; "
                            "move settings to channels.ws_proxy in config.yaml",
                            DeprecationWarning,
                            stacklevel=2,
                        )
                        cfg = old_cfg
    ...
    async def start(self, agent: "Agent", **kwargs: Any) -> None:
        ...
        Args:
            ...
            **kwargs: Expects ``app`` (MarchApp) for shared session store access.
        """
        ...
        # Load config from app if not already configured via constructor
        if app and self._config is None:
            self._load_config(app)

        # Use the app's shared SessionStore via ChatDB adapter
        session_store = kwargs.get("session_store") or (app.session_store if app else None)
        if session_store:
            self._db = ChatDB(store=session_store)
        else:
    ...
    async def send(self, content: str, **kwargs: Any) -> None:
        """Send a complete response to a specific WebSocket session.

        Args:
            content: The text content to send.
            **kwargs: Expects ``session_id`` to target a specific connection.
        """
        session_id = kwargs.get("session_id")
        if session_id and session_id in self._active_connections:
            ws = self._active_connections[session_id]
            await _try_send(ws, {"type": "message", "content": content})

    async def send_stream(
        self, chunks: AsyncIterator["StreamChunk"], **kwargs: Any
    ) -> None:
        """Send a streaming response to a specific WebSocket session.
        ...
        """
        session_id = kwargs.get("session_id")
        if not session_id or session_id not in self._active_connections:
            return
        ws = self._active_connections[session_id]
        await _try_send(ws, {"type": "stream.start"})
        collected = ""
        async for chunk in chunks:
            delta = chunk.content if hasattr(chunk, "content") else str(chunk)
            collected += delta
            await _try_send(ws, {"type": "stream.delta", "content": delta})
        await _try_send(ws, {"type": "stream.end"})
    ...
    def _create_session_sync(self, name: str, description: str = "") -> dict:
        ...
        if self._db is None or self._db._store is None:
            raise ValueError("WSChannel DB not initialised")
        db_path = str(self._db._store.db_path)
    ...
    async def _stream_response(self, conn: "_WSConn", content: str | list) -> None:
        ...
        if self._orchestrator is None:
            raise ValueError("WSChannel._stream_response called before Orchestrator was initialised")
...
# Code that imports WSProxyPlugin will still work but should migrate to WSChannel.
WSProxyPlugin = WSChannel
